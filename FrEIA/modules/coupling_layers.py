...
class GLOWCouplingBlock(nn.Module):
    ...
    def e(self, s):
        if self.pres_vol:
            return torch.exp(self.volume(self.clamp * 2 * torch.tanh(s / self.clamp)))
        else:
            return torch.exp(self.clamp * 2 * torch.tanh(s / self.clamp))


    def log_e(self, s):
        if self.pres_vol:
            return self.volume(self.clamp * 2 * torch.tanh(s / self.clamp))
        else:
            return self.clamp * 2 * torch.tanh(s / self.clamp)

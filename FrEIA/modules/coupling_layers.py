...
class GLOWCouplingBlock(nn.Module):
    ...
    def __init__(self, dims_in, dims_c=[], subnet_constructor=None, clamp=5., preserve_volume=False):
        super().__init__()
    ...
    def volume(self,s):
        if len(s.shape)==4:
            s[:, -1, -1, -1] = -torch.sum(s, tuple(range(1, len(s.shape)))) + s[:, -1, -1, -1]
            return s

        if len(s.shape)==3:
            s[:, -1, -1] = -torch.sum(s, tuple(range(1, len(s.shape)))) + s[:, -1, -1]
            return s

        if len(s.shape)==2:
            s[:, -1] = -torch.sum(s, tuple(range(1, len(s.shape)))) + s[:, -1]
            return s

    def e(self, s):
        if self.pres_vol:
            return torch.exp(self.volume(self.clamp * 0.636 * torch.atan(s / self.clamp)))
        else:
            torch.exp(self.clamp * 0.636 * torch.atan(s / self.clamp))


    def log_e(self, s):
        if self.pres_vol:
            return self.volume(self.clamp * 0.636 * torch.atan(s / self.clamp))
        else:
            self.clamp * 0.636 * torch.atan(s / self.clamp)

...
class GINCouplingBlock(nn.Module):
    ...
    def forward(self, x, c=[], rev=False):                
        if not rev:
            s2 = (self.s2(x[0].narrow(1, self.split_len1, self.split_len2)))[:, :self.split_len1]
            s2 = self.log_e(s2)
            s2 -= s2.mean(1, keepdim=True)

            y1 = (torch.exp(s2) * (x[0].narrow(1, 0, self.split_len1)) + (self.s2(x[0].narrow(1, self.split_len1, self.split_len2)))[:, self.split_len1:])

            s1 = (self.s1(y1))[:, :self.split_len2]
            s1 = self.log_e(s1)
        ...
        return [torch.cat((y1, (torch.exp(s1) * (x[0].narrow(1, self.split_len1, self.split_len2)) + (self.s1(y1))[:, self.split_len2:])), 1)]

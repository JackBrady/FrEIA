...
class GINCouplingBlock(nn.Module):
    ...
    def forward(self, x, c=[], rev=False):
        ...
        if not rev:
            s2, t2 = (self.log_e(x2[:, :self.split_len1])-s2.mean(1, keepdim=True)), x2[:, self.split_len1:]
            s1, t1 = self.log_e((torch.exp(s2) * x1 + t2)[:, :self.split_len2])-s1.mean(1, keepdim=True), (torch.exp(s2) * x1 + t2)[:, self.split_len2:]
        ...
        return [torch.cat(((torch.exp(s2) * x1 + t2), (torch.exp(s1) * x2 + t1)), 1)]

...
class GINCouplingBlock(nn.Module):
    ...
    def __init__(self, dims_in, dims_c=[], subnet_constructor=None, clamp=5.):
        ...
        self.s1 = subnet_constructor(self.split_len1, self.split_len2*2)
        self.s2 = subnet_constructor(self.split_len2, self.split_len1*2)
    ...
    def forward(self, x, c=[], rev=False):
        ...
        if not rev:
            ...
            s2= self.log_e(x2[:, :self.split_len1])
            s2 -= s2.mean(1, keepdim=True)

            s1 = (torch.exp(s2) * x1 + x2[:, self.split_len1:])[:, :self.split_len2]
            s1 = self.log_e(s1)
        ...
        return [torch.cat(((torch.exp(s2) * x1 + x2[:, self.split_len1:]), torch.exp(s1) * x2 + (torch.exp(s2) * x1 + x2[:, self.split_len1:])[:, self.split_len2:]), 1)]

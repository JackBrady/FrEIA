...
class GLOWCouplingBlock(nn.Module):
    ...
    def e(self, s):
        if self.pres_vol:
            return torch.exp(self.preserve_volume(self.clamp * 0.636 * torch.atan(s / self.clamp)))
        else:
    ...
    def log_e(self, s):
        if self.pres_vol:
            return self.preserve_volume(self.clamp * 0.636 * torch.atan(s / self.clamp))
        else:
            return self.clamp * 0.636 * torch.atan(s / self.clamp)

    def preserve_volume(self,s):
        if len(s)==4:
            s[:, -1, -1, -1] = -torch.sum(s, tuple(range(1, len(s.shape)))) + s[:, -1, -1, -1]
            print(torch.sum(s,tuple(range(1, len(s.shape)))))
